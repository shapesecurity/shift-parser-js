...
suite('Parser', function () {
  ...
  suite('early grammar errors', function () {
    ...
    // It is an early Reference Error if LeftHandSideExpression is neither an ObjectLiteral nor
    // an ArrayLiteral and IsValidSimpleAssignmentTarget of LeftHandSideExpression is false.
    testParseFailure('0 = 0;', 'Invalid left-hand side in assignment');
    ...
    // : ( Expression ) and Expression derives a production that would produce a Syntax Error according to these rules
    // if that production is substituted for LeftHandSideExpression. This rule is recursively applied.
    testParseFailure('for(({a: 0}) in 0);', 'Invalid left-hand side in for-in');
    testParseFailure('for(([0]) in 0);', 'Invalid left-hand side in for-in');
    testParseFailure('for(({a: 0}) of 0);', 'Invalid left-hand side in for-of');
    testParseFailure('for(([0]) of 0);', 'Invalid left-hand side in for-of');
  ...
  suite('early tokenisation errors', function () {
    ...
    // 11.6.1.1
    // It is a Syntax Error if SV(UnicodeEscapeSequence) is neither the UTF16Encoding (10.1.1) of a
    // single Unicode code point with the Unicode property “ID_Start” nor "$" or "_".
    testParseFailure('\\u0000', 'Unexpected end of input');
    testParseFailure('\\u{0}', 'Unexpected end of input');
    // It is a Syntax Error if SV(UnicodeEscapeSequence) is neither the UTF16Encoding (10.1.1) of a single
    // Unicode code point with the Unicode property “ID_Continue” nor "$" or "_" nor the UTF16Encoding
  ...
  suite('early errors', function () {
    ...
    testEarlyError('\'use strict\'; arguments = 0',
      'The identifier "arguments" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; arguments *= 0',
      'The identifier "arguments" must not be in binding position in strict mode');

    testEarlyError('\'use strict\'; [eval] = 0',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; [,,,eval,] = 0',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ({a: eval} = 0)',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ({a: eval = 0} = 0)',
      'The identifier "eval" must not be in binding position in strict mode');
    ...
    testEarlyError('\'use strict\'; [,,,arguments,] = 0',
      'The identifier "arguments" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ({a: arguments} = 0)',
      'The identifier "arguments" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ({a: arguments = 0} = 0)',
      'The identifier "arguments" must not be in binding position in strict mode');

    testEarlyError('\'use strict\'; var eval;',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; var arguments;',
      'The identifier "arguments" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; let [eval] = 0;',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; const {a: eval} = 0;',
      'The identifier "eval" must not be in binding position in strict mode');
    testModuleEarlyError('var eval;',
      'The identifier "eval" must not be in binding position in strict mode');
    ...
    testEarlyError('\'use strict\'; function f(eval){}',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('function f(eval){ \'use strict\'; }',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; !function (eval){}',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('!function (eval){ \'use strict\'; }',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; function* f(eval){}',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('function* f(eval){ \'use strict\'; }',
    ...
    testEarlyError('!function* (eval){ \'use strict\'; }',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('!{ f(eval){ \'use strict\'; } };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('!{ *f(eval){ \'use strict\'; } };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; !{ set f(eval){} };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('!{ set f(eval){ \'use strict\'; } };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('class A { f(eval){} };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('class A { *f(eval){} };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('class A { set f(eval){} };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('class A extends (eval = null) { };',
     'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('!class extends (eval = null) { };',
     'The identifier "eval" must not be in binding position in strict mode');
    // It is a Syntax Error if the code matched by this production is contained in strict code.
    testEarlyError('\'use strict\'; +yield;',
     'The identifier "yield" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; yield:;',
     'The identifier "yield" must not be in label position in strict mode');
    testEarlyError('\'use strict\'; var [yield] = 0;',
     'The identifier "yield" must not be in binding position in strict mode');
    // It is a Syntax Error if this phrase is contained in strict code and the StringValue of
    // IdentifierName is: "implements", "interface", "let", "package", "private", "protected",
    // "public", "static", or "yield".
    testEarlyError('\'use strict\'; +implements;',
     'The identifier "implements" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +interface;',
     'The identifier "interface" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +let;',
     'The identifier "let" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +package;',
     'The identifier "package" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +private;',
     'The identifier "private" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +protected;',
     'The identifier "protected" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +public;',
     'The identifier "public" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +static;',
     'The identifier "static" must not be in expression position in strict mode');
    testEarlyError('\'use strict\'; +yield;',
     'The identifier "yield" must not be in expression position in strict mode');
    testEarlyError('function a(yield){ \'use strict\'; }',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('function a(){ \'use strict\'; function a(a=yield){}}',
     'The identifier "yield" must not be in expression position in strict mode');
    testEarlyError('function a(){ \'use strict\'; function a(yield){}}',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; function a([yield]){}',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; function a({yield}){}',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; function a({yield=0}){}',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; function a({a:yield}){}',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; function a([yield,...a]){}',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('class A {set a(yield){}}', 'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('package => {\'use strict\'}',
     'The identifier "package" must not be in binding position in strict mode');
    testEarlyError('(package) => {\'use strict\'}',
     'The identifier "package" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ([let]) => {}',
     'The identifier "let" must not be in binding position in strict mode');
    testEarlyError('({a(yield){ \'use strict\'; }})',
     'The identifier "yield" must not be in binding position in strict mode');
    testEarlyError('!{ get a() { \'use strict\'; +let; } }',
     'The identifier "let" must not be in expression position in strict mode');
    testEarlyError('!{ set a(let) { \'use strict\'; } }',
     'The identifier "let" must not be in binding position in strict mode');
    testEarlyError('!{ a(let) { \'use strict\'; } }',
     'The identifier "let" must not be in binding position in strict mode');
    testEarlyError('!{ a(let) { \'use strict\'; } }',
     'The identifier "let" must not be in binding position in strict mode');
    ...
    testEarlyError('({ a(){ super(); } });', 'Calls to super must be in the "constructor" method of a class ' +
      'expression or class declaration that has a superclass');
    testEarlyError('({ a(){ {{ if(0) (( super() )); }} } });', 'Calls to super must be in the "constructor" ' +
      'method of a class expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { !{constructor() { super(); }}; } }', 'Calls to super must be ' +
      'in the "constructor" method of a class expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { !{*constructor() { super(); }}; } }', 'Calls to super must ' +
      'be in the "constructor" method of a class expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { !{get constructor() { super(); }}; } }', 'Calls to super ' +
      'must be in the "constructor" method of a class expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { !{set constructor(a) { super(); }}; } }', 'Calls to super ' +
      'must be in the "constructor" method of a class expression or class declaration that has a superclass');
    ...
    // 12.5.4.1
    // It is a Syntax Error if the UnaryExpression is contained in strict code and the derived UnaryExpression
    // is PrimaryExpression : IdentifierReference.
    ...
    testEarlyError('\'use strict\'; ({eval} = 0);',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ({eval = 0} = 0);',
      'The identifier "eval" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ({arguments} = 0);',
      'The identifier "arguments" must not be in binding position in strict mode');
    testEarlyError('\'use strict\'; ({arguments = 0} = 0);',
      'The identifier "arguments" must not be in binding position in strict mode');
    ...
    testEarlyError('if(0) label: function f(){}',
      'The consequent of an if statement must not be a labeled function declaration');
    testEarlyError('if(0) labelA: labelB: function f(){}',
      'The consequent of an if statement must not be a labeled function declaration');
    testEarlyError('if(0) label: function f(){} else ;',
      'The consequent of an if statement must not be a labeled function declaration');
    testEarlyError('if(0) ; else label: function f(){}',
      'The alternate of an if statement must not be a labeled function declaration');

    // 13.7.1.1
    // It is a Syntax Error if IsLabelledFunction(Statement) is true for any occurrence of Statement in these rules.
    testEarlyError('do label: function f(){} while (0)',
      'The body of a do-while statement must not be a labeled function declaration');
    testEarlyError('do label: function f(){} while (0);',
      'The body of a do-while statement must not be a labeled function declaration');
    testEarlyError('while(0) label: function f(){}',
      'The body of a while statement must not be a labeled function declaration');
    testEarlyError('for(;;) label: function f(){}',
      'The body of a for statement must not be a labeled function declaration');
    testEarlyError('for(var a;;) label: function f(){}',
      'The body of a for statement must not be a labeled function declaration');
    testEarlyError('for(const a = 0;;) label: function f(){}',
      'The body of a for statement must not be a labeled function declaration');
    testEarlyError('for(let a;;) label: function f(){}',
      'The body of a for statement must not be a labeled function declaration');
    testEarlyError('for(a in b) label: function f(){}',
      'The body of a for-in statement must not be a labeled function declaration');
    testEarlyError('for(var a in b) label: function f(){}',
      'The body of a for-in statement must not be a labeled function declaration');
    testEarlyError('for(let a in b) label: function f(){}',
      'The body of a for-in statement must not be a labeled function declaration');
    testEarlyError('for(const a in b) label: function f(){}',
      'The body of a for-in statement must not be a labeled function declaration');
    testEarlyError('for(a of b) label: function f(){}',
      'The body of a for-of statement must not be a labeled function declaration');
    testEarlyError('for(var a of b) label: function f(){}',
      'The body of a for-of statement must not be a labeled function declaration');
    testEarlyError('for(let a of b) label: function f(){}',
      'The body of a for-of statement must not be a labeled function declaration');
    testEarlyError('for(const a of b) label: function f(){}',
      'The body of a for-of statement must not be a labeled function declaration');
    testEarlyError('for(;;) labelA: labelB: labelC: function f(){}',
      'The body of a for statement must not be a labeled function declaration');
    ...
    testEarlyError('while(0) !function(){ continue; };',
      'Continue statement must be nested within an iteration statement');
    testEarlyError('while(0) { function f(){ continue; } }',
      'Continue statement must be nested within an iteration statement');
    testEarlyError('label: continue label;',
      'Continue statement must be nested within an iteration statement with label "label"');
    testEarlyError('label: { continue label; }',
      'Continue statement must be nested within an iteration statement with label "label"');
    testEarlyError('label: if(0) continue label;',
      'Continue statement must be nested within an iteration statement with label "label"');
    ...
    testEarlyError('with(0) label: function f(){}',
      'The body of a with statement must not be a labeled function declaration');
    ...
    testEarlyError('function f(a = super()){}', 'Calls to super must be in the "constructor" method of a ' +
      'class expression or class declaration that has a superclass');
    testModuleEarlyError('export default function(a = super()){}', 'Calls to super must be in the "constructor" ' +
      'method of a class expression or class declaration that has a superclass');
    testEarlyError('!function(a = super()){}', 'Calls to super must be in the "constructor" method of a class ' +
      'expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { function f(a = super()){} } }', 'Calls to super must be ' +
      'in the "constructor" method of a class expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { !function(a = super()){} } }', 'Calls to super must be in the ' +
      '"constructor" method of a class expression or class declaration that has a superclass');
    // It is a Syntax Error if FunctionBody Contains SuperCall is true.
    testEarlyError('function f(a){ super() }', 'Calls to super must be in the "constructor" method of a class ' +
      'expression or class declaration that has a superclass');
    testModuleEarlyError('export default function(a){ super() }', 'Calls to super must be in the "constructor" method ' +
      'of a class expression or class declaration that has a superclass');
    testEarlyError('!function(a){ super() }', 'Calls to super must be in the "constructor" method of a class ' +
      'expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { function f(){ super(); } } }', 'Calls to super must be ' +
      'in the "constructor" method of a class expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { !function(){ super(); } } }', 'Calls to super must be in the ' +
      '"constructor" method of a class expression or class declaration that has a superclass');
    ...
    testEarlyError('(a,...a)=>0', 'Duplicate binding "a"');
    testEarlyError('([a],...a)=>0', 'Duplicate binding "a"');
    // It is a Syntax Error if IsSimpleParameterList of FormalParameterList is false and BoundNames
    ...
    testEarlyError('function f(){ break label; }',
      'Break statement must be nested within a statement with label "label"');
    ...
    // sequence matched by CoverParenthesizedExpressionAndArrowParameterList[?Yield] cannot be parsed with no tokens
    // left over using ArrowFormalParameters[Yield, GeneratorParameter] as the goal symbol.
    // TODO
    ...
    // TODO
    // All early errors rules for ArrowFormalParameters and its derived productions also apply to CoveredFormalsList
    // of CoverParenthesizedExpressionAndArrowParameterList[?Yield].
    // TODO
    ...
    testEarlyError('!{ *f(a = super()){} };', 'Calls to super must be in the "constructor" method of a class ' +
      'expression or class declaration that has a superclass');
    testEarlyError('!{ *f(a) { super() } };', 'Calls to super must be in the "constructor" method of a class ' +
      'expression or class declaration that has a superclass');
    // It is a Syntax Error if StrictFormalParameters Contains YieldExpression is true.
    testEarlyError('function* g(){ ({ *m(a = yield){} }); }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ ({ *m(a = yield b){} }); }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ ({ *m(a = yield* b){} }); }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ ({ *m(a = x + f(yield)){} }); }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ ({ *m({[yield]: a}){} }); }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ ({ *m({a = yield}){} }); }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ ({ *m([a = yield]){} }); }',
    ...
    testEarlyError('function* f(a){ super() }', 'Calls to super must be in the "constructor" method of a ' +
      'class expression or class declaration that has a superclass');
    testEarlyError('class A extends B { constructor() { function* f(){ super(); } } }', 'Calls to super ' +
      'must be in the "constructor" method of a class expression or class declaration that has a superclass');
    ...
    testEarlyError('function* g(){ function* f(a = yield){} }',
      'Generator parameters must not contain yield expressions');
    ...
    // "Generator parameters must not contain yield expressions");
    testEarlyError('function* g(){ !function*(a = yield){} }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ !function*(a = yield b){} }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ !function*(a = yield* b){} }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ !function*(a = x + f(yield)){} }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ !function*({[yield]: a}){} }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ !function*({a = yield}){} }',
      'Generator parameters must not contain yield expressions');
    testEarlyError('function* g(){ !function*([a = yield]){} }',
    ...
    testEarlyError('class A { constructor() { super(); } }', 'Calls to super must be in the "constructor" method ' +
      'of a class expression or class declaration that has a superclass');
    testEarlyError('class A { constructor() { {{ (( super() )); }} } }', 'Calls to super must be in the "constructor" ' +
      'method of a class expression or class declaration that has a superclass');
    ...
    testEarlyError('!class A { constructor(){} constructor(){} }', 'Duplicate constructor method in class');
    testEarlyError('!class A { constructor(){} "constructor"(){} }', 'Duplicate constructor method in class');
    // It is a Syntax Error if PropName of MethodDefinition is not "constructor" and HasDirectSuper
    // of MethodDefinition is true.
    testEarlyError('class A extends B { f() { super(); } }', 'Calls to super must be in the "constructor" method ' +
      'of a class expression or class declaration that has a superclass');
    ...
    // It is a Syntax Error if HasDirectSuper of MethodDefinition is true.
    testEarlyError('class A extends B { static f() { super(); } }', 'Calls to super must be in the "constructor" ' +
      'method of a class expression or class declaration that has a superclass');
    // It is a Syntax Error if PropName of MethodDefinition is "prototype".
    testEarlyError('class A extends B { static prototype(){} }', 'Static class methods cannot be named "prototype"');
    testEarlyError('class A extends B { static *prototype(){} }', 'Static class methods cannot be named "prototype"');
    testEarlyError('class A extends B { static get prototype(){} }',
      'Static class methods cannot be named "prototype"');
    testEarlyError('class A extends B { static set prototype(a) {} }',
      'Static class methods cannot be named "prototype"');
    ...
    // is being processed by a direct eval that is contained in function code. However, such function code does not
    // include ArrowFunction function code.
    testEarlyError('super()', 'Calls to super must be in the "constructor" method of a class expression or class ' +
      'declaration that has a superclass');
    ...
    // eval code that is being processed by a direct eval that is contained in function code. However, such function
    // code does not include ArrowFunction function code.
    ...
    testEarlyError('while(0) continue label;',
     'Continue statement must be nested within an iteration statement with label "label"');
    ...
    // It is a Syntax Error if any element of the LexicallyDeclaredNames of ModuleItemList also occurs
    // in the VarDeclaredNames of ModuleItemList.
    ...
    testModuleEarlyError('let a; export default function a(){};', 'Duplicate binding "a"');
    testModuleEarlyError('let a; export default class a {};', 'Duplicate binding "a"');
    ...
    testModuleEarlyError('var a; export default function() {} export { a as default };', 'Duplicate export "default"');
    testModuleEarlyError('var a; export default class {} export { a as default };', 'Duplicate export "default"');
    testModuleEarlyError('var a, b; export default a; export { b as default };', 'Duplicate export "default"');
    ...
    testModuleEarlyError('super()', 'Calls to super must be in the "constructor" method of a class expression ' +
    ...
    testModuleEarlyError('while(0) continue label;',
     'Continue statement must be nested within an iteration statement with label "label"');
    ...
    // and at least two of those entries were obtained from productions of the form PropertyDefinition :
    // PropertyName : AssignmentExpression .
    testEarlyError('!{ __proto__: null, __proto__: null };',
      'Duplicate __proto__ property in object literal not allowed');
    testEarlyError('!{ __proto__: null, "__proto__": null };',
      'Duplicate __proto__ property in object literal not allowed');
    testEarlyError('!{ __proto__: null, __proto__: null, };',
      'Duplicate __proto__ property in object literal not allowed');
    ...
    testEarlyError('\'use strict\'; label: function f(){}',
      'Labeled FunctionDeclarations are disallowed in strict mode');

    // Annex B 3.4
    // The above rules are only applied when parsing code that is not strict mode code
    testEarlyError('\'use strict\'; if (0) function f(){}',
      'FunctionDeclarations in IfStatements are disallowed in strict mode');
    testEarlyError('\'use strict\'; if (0) function f(){} else;',
      'FunctionDeclarations in IfStatements are disallowed in strict mode');
    testEarlyError('\'use strict\'; if (0); else function f(){}',
      'FunctionDeclarations in IfStatements are disallowed in strict mode');
    ...
    // It is a Syntax Error if any element of the BoundNames of CatchParameter also occurs in the VarDeclaredNames of
    // Block, unless that element is only bound by a VariableStatement or the VariableDeclarationList of a for
    ...
    // is false.
    testEarlyError('function a([]){\'use strict\';}',
      'Functions with non-simple parameter lists may not contain a "use strict" directive');
    testEarlyError('(function ([]){\'use strict\';})',
      'Functions with non-simple parameter lists may not contain a "use strict" directive');
    testEarlyError('(([]) => {\'use strict\';})',
      'Functions with non-simple parameter lists may not contain a "use strict" directive');
    testEarlyError('({set a([]){\'use strict\'}})',
      'Functions with non-simple parameter lists may not contain a "use strict" directive');
    testEarlyError('({a([]){\'use strict\'}})',
      'Functions with non-simple parameter lists may not contain a "use strict" directive');
  });
