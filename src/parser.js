...
export class Parser extends Tokenizer {
  ...
  parseVariableIdentifier() {
    ...
    return this.markLocation(new Shift.Identifier(this.expect(TokenType.IDENTIFIER).value), startTokenIndex);
  }
    ...
    switch (node.type) {
      ...
      case "ShorthandProperty":
        return new Shift.BindingPropertyIdentifier(
          new Shift.BindingIdentifier(node.name),
          null
        );
  ...
  parseArrowExpressionTail(head, startTokenIndex) {
  ...
  parsePrimaryExpression() {
    ...
    switch (this.lookahead.type) {
      ...
      case TokenType.TRUE:
        this.lex();
        return this.markLocation(new Shift.LiteralBooleanExpression(true), startTokenIndex);
      case TokenType.FALSE:
        this.lex();
        return this.markLocation(new Shift.LiteralBooleanExpression(false), startTokenIndex);
      case TokenType.NULL:
        this.lex();
  ...
  parseGroupExpression() {
    ...
    if (!this.hasLineTerminatorBeforeNext && this.match(TokenType.ARROW)) {
      if (!possibleBindings) {
        throw this.createErrorWithToken(start, ErrorMessages.ILLEGAL_ARROW_FUNCTION_PARAMS);
      }
      // check dup params
  ...
  parseObjectPropertyKey() {
    ...
    return new Shift.StaticPropertyName(this.parseIdentifierName().name);
  }
  ...
  parseFunction(isExpression) {
    ...
    this.strict = previousStrict;
    return this.markLocation(new (isExpression ? Shift.FunctionExpression : Shift.FunctionDeclaration)(false, id, info.params, info.rest, body),
        startTokenIndex);
  }
